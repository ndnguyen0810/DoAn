...
namespace QuanLyBaoChi.FORM
{
	public partial class frmQuanTriAdmin : Form
	{
        ...
        //string chuoi = @"Data Source=.\SQLExpress;AttachDbFilename=QuanLyBaoChi_New.mdf; Initial Catalog=QuanLyBaoChi_New;Trusted_Connection=Yes";
        ...
        private void btnTC_Sua_Click(object sender, EventArgs e)
		{
            ...
            try
            {
                if (reg.IsMatch(txtNB_Email.Text))
                {
        ...
        private void btnNB_Sua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (reg.IsMatch(this.txtTC_Email.Text))
                {
                    if (NhaBaoDAO.Instance.SuaNhaBao(id, ten, but, sdt, email, dc))
                    {
                        MessageBox.Show("Sửa Thành Công", "Thông Báo");
                        LoadTableNhaBao();
                    }
        ...
        private void dgvTK_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                n_status.Value = Convert.ToInt32(row.Cells[4].Value.ToString());

...
namespace QuanLyBaoChi.DAO
{
     public class NhaBaoDAO
     {
        ...
        public bool SuaNhaBao(int id, string ten, string hieu, string email, string sdt, string dc)
        {
            string query = string.Format("UPDATE NhaBao SET TenNhaBao=N'{1}',ButHieu=N'{2}', sdt_NB={3},email_NB='{4}',diachi_NB=N'{5}' where ID_nhaBao={0}", id,ten, hieu, email, sdt, dc);
            int resual = DataProvider.Instance.ExcuteNonQuery(query);
            return resual > 0;
        }
